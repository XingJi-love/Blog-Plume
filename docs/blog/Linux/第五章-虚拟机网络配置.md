...
BROWSER_ONLY=no                              # 仅浏览器模式：否
BOOTPROTO=static                             # IP获取方式：静态（原dhcp改为static）
DEFROUTE=yes                                 # 将此接口设为默认路由：是
IPV4_FAILURE_FATAL=no                        # IPv4故障是否致命：否
IPV6INIT=yes                                 # 启用IPv6：是
IPV6_AUTOCONF=yes                            # IPv6自动配置：是
IPV6_DEFROUTE=yes                            # IPv6默认路由：是
IPV6_FAILURE_FATAL=no                        # IPv6故障是否致命：否
IPV6_ADDR_GEN_MODE=stable-privacy            # IPv6地址生成模式：稳定隐私
NAME=ens33                                   # 连接名称
UUID=fcedcfe5-238a-485d-a920-1533a968ce45    # 设备唯一标识符（系统生成）
DEVICE=ens33                                 # 物理设备名称（与文件名一致）
ONBOOT=yes                                   # 开机自动启用：是（原no改为yes）
```

*   以下选项,有则修改,无则增加

```shell
IPADDR=192.168.119.66                        # 静态IPv4地址
NETMASK=255.255.255.0                        # 子网掩码（对应/24）
GATEWAY=192.168.119.2                        # 默认网关地址
DNS1=8.8.8.8                                 # 首选DNS服务器（Google）
